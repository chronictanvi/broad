    ...
    <div className="min-h-screen flex flex-col md:flex-row">
      {agencies.map((agency) => (
        <motion.div
          ...
          onClick={() =>
            setActiveAgency(activeAgency === agency.id ? "jaxa" : agency.id)
          }
        >
          <div className="p-8 h-full flex flex-col">
            <div className="space-y-2 flex-grow">
              <div className="flex items-center justify-between">
                <p className={`text-sm tracking-wider ${agency.textColor}`}>
                  {agency.name}
                </p>
                <motion.div
                ...
                {activeAgency === agency.id && (
                  ...
                  >
                    {agency.title.map((line, index) => (
                      <p
                        key={index}
                        className={`text-4xl font-light ${agency.textColor}`}
                      >
                        {line}
                      </p>
                    ))}
